...
struct Migrations {
    static func register(in migrator: inout DatabaseMigrator) throws {
        ...
        // Migration v7: User Dashboard Widgets (Unified Widget System)
        migrator.registerMigration("v7_user_dashboard_widgets") { db in
            try db.create(table: SchemaDefinitions.UserDashboardWidgetTable.databaseTableName) {
                t in
                t.column(SchemaDefinitions.UserDashboardWidgetTable.id, .text).primaryKey()
                t.column(SchemaDefinitions.UserDashboardWidgetTable.name, .text).notNull()
                t.column(SchemaDefinitions.UserDashboardWidgetTable.type, .text).notNull()
                t.column(SchemaDefinitions.UserDashboardWidgetTable.size, .text).notNull()
                t.column(SchemaDefinitions.UserDashboardWidgetTable.position, .integer).notNull()
                    .defaults(to: 0)
                t.column(SchemaDefinitions.UserDashboardWidgetTable.configuration, .blob)  // JSON data
                t.column(SchemaDefinitions.UserDashboardWidgetTable.isVisible, .boolean).notNull()
                    .defaults(to: true)
                t.column(SchemaDefinitions.UserDashboardWidgetTable.createdAt, .date).notNull()
                t.column(SchemaDefinitions.UserDashboardWidgetTable.updatedAt, .date).notNull()
            }
        }

        // Migration v8: Analytics Widget System
        migrator.registerMigration("v8_analytics_widgets") { db in
            // Create analytics widget table (same schema as dashboard widgets)
            try db.create(table: SchemaDefinitions.AnalyticsWidgetTable.databaseTableName) { t in
                t.column(SchemaDefinitions.AnalyticsWidgetTable.id, .text).primaryKey()
                t.column(SchemaDefinitions.AnalyticsWidgetTable.name, .text).notNull()
                t.column(SchemaDefinitions.AnalyticsWidgetTable.type, .text).notNull()
                t.column(SchemaDefinitions.AnalyticsWidgetTable.size, .text).notNull()
                t.column(SchemaDefinitions.AnalyticsWidgetTable.position, .integer).notNull()
                    .defaults(to: 0)
                t.column(SchemaDefinitions.AnalyticsWidgetTable.configuration, .blob)
                t.column(SchemaDefinitions.AnalyticsWidgetTable.isVisible, .boolean).notNull()
                    .defaults(to: true)
                t.column(SchemaDefinitions.AnalyticsWidgetTable.createdAt, .date).notNull()
                t.column(SchemaDefinitions.AnalyticsWidgetTable.updatedAt, .date).notNull()
            }

            // Migrate existing charts from chartDefinition table to analytics widgets
            let existingCharts = try Row.fetchAll(
                db,
                sql: "SELECT * FROM chartDefinition ORDER BY sortOrder ASC"
            )

            let now = ISO8601DateFormatter().string(from: Date())

            for (index, chart) in existingCharts.enumerated() {
                guard
                    let title = chart["title"] as? String,
                    chart["chartType"] is String
                else {
                    continue  // Skip malformed charts
                }

                // Determine widget type based on chart title
                let widgetType: String
                if title.contains("Revenue") {
                    widgetType = "revenue_chart"
                } else if title.contains("Profit") {
                    widgetType = "profit_chart"
                } else if title.contains("Category") || title.contains("Categories") {
                    widgetType = "top_categories"
                } else if title.contains("Product") || title.contains("Brand") {
                    widgetType = "top_brands"
                } else {
                    widgetType = "revenue_chart"  // Default fallback
                }

                // Insert as analytics widget (configuration can be added later)
                try db.execute(
                    sql: """
                        INSERT INTO user_analytics_widget
                        (id, name, type, size, position, isVisible, createdAt, updatedAt)
                        VALUES (?, ?, ?, ?, ?, 1, ?, ?)
                        """,
                    arguments: [
                        UUID().uuidString,
            ...
            print("✅ Migrated \\(existingCharts.count) charts to analytics widgets")
        }
